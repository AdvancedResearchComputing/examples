#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on Tinkercliffs (amd nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_TINKERCLIFFS amd: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS amd: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS amd: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_tinkercliffs_amd.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS amd: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_tinkercliffs_amd.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_tinkercliffs_amd.txt
echo "=====================" >> netcdf_tinkercliffs_amd.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_tinkercliffs_amd.txt
...
echo "NETCDF_TINKERCLIFFS amd: Finished execution successfully."
exit 0
