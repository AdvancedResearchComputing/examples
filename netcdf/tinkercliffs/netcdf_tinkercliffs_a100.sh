#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on Tinkercliffs (A100 nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_TINKERCLIFFS A100: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS A100: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS A100: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_tinkercliffs_A100.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS A100: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_tinkercliffs_A100.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_tinkercliffs_A100.txt
echo "=====================" >> netcdf_tinkercliffs_A100.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_tinkercliffs_A100.txt
...
echo "NETCDF_TINKERCLIFFS A100: Finished execution successfully."
exit 0
