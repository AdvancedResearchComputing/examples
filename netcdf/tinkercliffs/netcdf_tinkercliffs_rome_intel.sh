#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on Tinkercliffs (intel nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_TINKERCLIFFS intel: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS intel: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS intel: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_tinkercliffs_intel.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_TINKERCLIFFS intel: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_tinkercliffs_intel.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_tinkercliffs_intel.txt
echo "=====================" >> netcdf_tinkercliffs_intel.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_tinkercliffs_intel.txt
...
echo "NETCDF_TINKERCLIFFS intel: Finished execution successfully."
exit 0
