#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on OWL (milan nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_OWL milan: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL milan: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL milan: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_OWL_milan.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL milan: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_OWL_milan.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_OWL_milan.txt
echo "=====================" >> netcdf_OWL_milan.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_OWL_milan.txt
...
echo "NETCDF_OWL milan: Finished execution successfully."
exit 0
