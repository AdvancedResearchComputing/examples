#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on OWL (genoa nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_OWL genoa: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL genoa: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL genoa: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_OWL_genoa.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_OWL genoa: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_OWL_genoa.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_OWL_genoa.txt
echo "=====================" >> netcdf_OWL_genoa.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_OWL_genoa.txt
...
echo "NETCDF_OWL genoa: Finished execution successfully."
exit 0
