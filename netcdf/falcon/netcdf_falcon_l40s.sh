#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on Falcon (L40s nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_Falcon L40s: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_Falcon L40s: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_Falcon L40s: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_falcon_L40s.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_FALCON L40s: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_falcon_L40s.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_falcon_L40s.txt
echo "=====================" >> netcdf_falcon_L40s.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_falcon_L40s.txt
...
echo "NETCDF_Falcon L40s: Finished execution successfully."
exit 0
