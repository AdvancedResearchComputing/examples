#! /bin/bash
#
# SLURM Job Script to Compile, Run, and Demonstrate netCDF Utilities on Falcon (T4 nodes)
#
#SBATCH -t 00:10:00
#SBATCH -N1
...
echo "NETCDF_Falcon T4: Starting execution."
...
if [ $? -ne 0 ]; then
  echo "NETCDF_Falcon T4: Compilation failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "NETCDF_Falcon T4: Linking failed!"
  exit 1
fi
...
./netcdf_test > netcdf_falcon_T4.txt
if [ $? -ne 0 ]; then
  echo "NETCDF_FALCON T4: Execution failed!"
  exit 1
fi
...
echo "=== ncdump output ===" >> netcdf_falcon_T4.txt
$EBROOTNETCDF/bin/ncdump netcdf_test.nc >> netcdf_falcon_T4.txt
echo "=====================" >> netcdf_falcon_T4.txt
...
echo "Created netCDF copy: netcdf_test_copy.nc" >> netcdf_falcon_T4.txt
...
echo "NETCDF_Falcon T4: Finished execution successfully."
exit 0
